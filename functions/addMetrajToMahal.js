exports = async function (request, response) {
  ...
  try{
    ...
    const pozMahaller = await colMahaller.find({pozlar: {$elemMatch: {pozId}}}).toArray()
    const mongoReply = pozMahaller
  ...
  try{
    ...
    return ({hata:true,hataYeri:"FONK // addMetrajToMahal // MONGO-4",hataMesaj:"Gelen sorguda gövde(body) verisi yok ya da hatalı, MONGO --> " + err.message})
  }
  ...
  FonkGelenVeri: try {
    ...
    // let sTr = proje + "-" + "lbs"
    // if (!user.yetkiler[sTr].includes("W")) return ({hata:true,hataTanim:"yetki",hataYeri:"FONK // addMetrajToMahal",hataMesaj:"Bu alana veri kaydetme yetkiniz bulunmuyor"})
    ...
    // if (checkSira_Ekle) return ({hata:true,hataYeri:"FONK // addMetrajToMahal",hataMesaj:"Sira numarası verilmemiş kayıt eklenemez.."}) 
    // if (checkSira_Guncelle) return ({hata:true,hataYeri:"FONK // addMetrajToMahal",hataMesaj:"Sira numarası silinemez"}) 
    // if (checkIsim_Ekle) return ({hata:true,hataYeri:"FONK // addMetrajToMahal",hataMesaj:"Lbs ismi boş bırakılamaz"}) 
    ...
    //   })
    // }
    // if (!checkSilinebilir) return ({hata:true,hataYeri:"FONK // addMetrajToMahal",hataMesaj:"Silmek istediğiniz başlığ1 bağlı mahaller var, öncelikle onları silmelisiniz"}) 
    ...
    return ({hata:true,hataYeri:"FONK // addMetrajToMahal // MONGO-5",hataMesaj:err.message});
  }
  ...
  try {
    ...
    return ({hata:true,hataYeri:"FONK // addMetrajToMahal // MONGO-7",hataMesaj:err.message});
  }        
