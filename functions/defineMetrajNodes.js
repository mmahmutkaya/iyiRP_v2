exports = async function (request, response) {
  ...
  Fonk_Define: try {
    ...
    if (isKayitYapilabilirProblemi_define) return ({hata:true,hataYeri:"FONK // defineMetrajNodes // MONGO-5",hataMesaj:"İlgili ihalenin şu anda mahal-poz eşleştirmesi veri güncellemesine açık değil fakat \"YENİLE\" tuşuna basarak mevcut eşleştirmeleri görebilirsiniz."});
    if (yazmaYetkisiProblemi_define) return ({hata:true,hataYeri:"FONK // defineMetrajNodes // MONGO-5",hataMesaj:"İlgili alana keşif metraj kaydetme yetkiniz bulunmuyor."});
  ...
  Fonk_Tur: try {
    ...
    if (isKayitYapilabilirProblemi_tur) return ({hata:true,hataYeri:"FONK // defineMetrajNodes // MONGO-5",hataMesaj:"İlgili ihale şu anda \"" + tur + "\" metraj girişine açık değil fakat \"YENİLE\" tuşuna basarak varsa mevcut \"" + tur + "\" metrajlarını görebilirsiniz."});
    if (yazmaYetkisiProblemi_tur) return ({hata:true,hataYeri:"FONK // defineMetrajNodes // MONGO-5",hataMesaj:"İlgili alana  \"" + tur + "\" metraj kaydetme yetkiniz bulunmuyor."});
    ...
    if (silinemez_mahal_pozIds.length) {
      return ({hata:true,hataYeri:"FONK // defineMetrajNodes",hataMesaj: silinemez_mahal_pozIds[0].pozNo + " - numaralı pozun metranısı silmek için \"standart\" metraj sayfalarını kullanmalısınız."}) 
    }
