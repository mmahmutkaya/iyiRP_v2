exports = async function (request, response) {
  ...
  try {
    ...
    if(!objHeader.hasOwnProperty('Ihale-Id')) return ({hata:true,hataYeri:"FONK // addLbs",hataMesaj:"Gelen sorguda \"Ihale-Id\" HEADER tespit edilemedi, yönetici ile iletişime geçiniz."})
    ihaleId = objHeader["Ihale-Id"][0];
    if(ihaleId.length == 0) return ({hata:true,hataYeri:"FONK // addLbs",hataMesaj:"Gelen sorguda \"Ihale-Id\" HEADER var fakat boş, yönetici ile iletişime geçiniz."})

    if(!objHeader.hasOwnProperty('Parent-Tur')) return ({hata:true,hataYeri:"FONK // addLbs",hataMesaj:"Gelen sorguda \"Parent-Tur\" HEADER tespit edilemedi, yönetici ile iletişime geçiniz."})
    parentTur = objHeader["Parent-Tur"][0];
    if(parentTur.length == 0) return ({hata:true,hataYeri:"FONK // addLbs",hataMesaj:"Gelen sorguda \"Parent-Tur\" HEADER var fakat boş, yönetici ile iletişime geçiniz."})

    // if(!objHeader.hasOwnProperty('Tur')) return ({hata:true,hataYeri:"FONK // addLbs",hataMesaj:"Gelen sorguda \"Tur\" HEADER tespit edilemedi, yönetici ile iletişime geçiniz."})
    // tur = objHeader["Tur"][0];
    // if(tur.length == 0) return ({hata:true,hataYeri:"FONK // addLbs",hataMesaj:"Gelen sorguda \"Tur\" HEADER var fakat boş, yönetici ile iletişime geçiniz."})
  ...
  try {
    ...
    const NodesArray = await collection.find({ihaleId:new BSON.ObjectId(ihaleId),isDeleted:false,tur:parentTur},{sira:1,isim:1,lastUpdatedChild:1}).sort({sira: 1}).toArray()
